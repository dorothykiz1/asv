...
def _parse_matrix(matrix, bare_keys=()):
    ...
    if any(t in matrix for t in matrix_types):
        ...
        other_keys = matrix_types + tuple(bare_keys)
        if remaining_keys:
            raise util.UserError(f'Unknown keys in "matrix" configuration:'
                                 f'{remaining_keys}, expected: {other_keys}')
    else:
...
class ExistingEnvironment(Environment):
    ...
    def run(self, args, **kwargs):
        self.args = ' '.join(args)
        log.debug(f"Running '{ self.args}' in {self.name}")
